 ...
 * {@link Migrations} and an additional bean of type {@link ServiceStartBuildItem}, the
 * latter indicating that all migrations have been applied (in case they are actually
 * enabled).
 *
...
public class MigrationsProcessor {
	...
	static Set<Class<? extends JavaBasedMigration>> findClassBasedMigrations(Collection<String> packagesToScan,
			IndexView indexView) {
		...
		indexView.getAllKnownImplementations(DotName.createSimple(JavaBasedMigration.class.getName())).forEach(cf -> {
			if (!packagesToScan.contains(cf.name().packagePrefix())) {
				return;
			}
			try {
				classesFoundAndLoaded.add(Thread.currentThread()
	...
	static Set<ResourceWrapper> findResourceBasedMigrations(Collection<ResourceBasedMigrationProvider> providers,
			Collection<String> locationsToScan) throws IOException {
		...
		// This piece is deliberately not using the streams due to the heckmeck with
		...
		for (var value : locationsToScan) {
			...
			ClassPathUtils.consumeAsPaths(name, rootResource -> {
				try (var paths = Files.walk(rootResource)) {
					paths.filter(isSupportedFile)
						// Resolving the string and not the path object is done on
	...
	@BuildStep
	@SuppressWarnings("unused")
	DiscovererBuildItem createDiscoverer(CombinedIndexBuildItem combinedIndexBuildItem,
	...
	ReflectiveClassBuildItem registerMigrationsForReflections(DiscovererBuildItem discovererBuildItem) {
		...
		return ReflectiveClassBuildItem
			.builder(discovererBuildItem.getDiscoverer().getMigrationClasses().toArray(new Class<?>[0]))
			.constructors()
			.methods()
			.fields()
			.build();
	...
	NativeImageResourceBuildItem addCypherResources(
		...
		return new NativeImageResourceBuildItem(classpathResourceScannerBuildItem.getScanner()
			.getResources()
			.stream()
	...
	@SuppressWarnings("unused")
	MigrationsBuildItem createMigrations(MigrationsBuildTimeProperties buildTimeProperties,
			DiscovererBuildItem discovererBuildItem,
			ClasspathResourceScannerBuildItem classpathResourceScannerBuildItem, MigrationsRecorder migrationsRecorder,
			Neo4jDriverBuildItem driverBuildItem, BuildProducer<SyntheticBeanBuildItem> syntheticBeans) {

		var configRv = migrationsRecorder.recordConfig(buildTimeProperties, discovererBuildItem.getDiscoverer(),
				classpathResourceScannerBuildItem.getScanner());
		syntheticBeans.produce(SyntheticBeanBuildItem.configure(MigrationsConfig.class)
			.runtimeValue(configRv)
			.setRuntimeInit()
			.done());
	...
	@Record(ExecutionTime.RUNTIME_INIT)
	@SuppressWarnings("unused")
	ServiceStartBuildItem applyMigrations(MigrationsRecorder migrationsRecorder,
			MigrationsBuildItem migrationsBuildItem) {
		migrationsRecorder.applyMigrations(migrationsBuildItem.getValue(), migrationsRecorder.isEnabled());
