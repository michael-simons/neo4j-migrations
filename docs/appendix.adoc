...
* A `<catalog />` consists of zero or one `<constraints />` and zero or one `<indexes />` elements.
...
The XMl schema for <<concepts_migrations_catalog-based, catalog-based migrations>> looks like this:
