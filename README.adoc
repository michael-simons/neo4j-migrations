...
* Core API: `{groupId}:{artifactIdCore}:{version}`
* Spring-Boot-Starter: `{groupId}:{artifactIdSpringBoot}:{version}`
* Quarkus extension: `{groupId}:{artifactIdQuarkus}:{version}`
* Maven-Plugin: `{groupId}:{artifactIdMavenPlugin}:{version}`
...
The API documentation for the core module is available here: https://michael-simons.github.io/neo4j-migrations/main/site/neo4j-migrations/apidocs/index.html[Neo4j Migrations (Core) {version} API].
