...
https://repo1.maven.org/maven2/eu/michael-simons/neo4j/neo4j-migrations/{latest_version}/neo4j-migrations-{latest_version}.zip[neo4j-migrations-{latest_version}.zip]
...
Database: Neo4j/4.0.0@localhost:7687 connected via bolt://localhost:7687
...
That starter itself depends on the starter for the Neo4j Java Driver: 
https://github.com/neo4j/neo4j-java-driver-spring-boot-starter[neo4j-java-driver-spring-boot-starter]
and thus you have auto configuration for your connection and the migrations.
...
After installing at least GraalVM 19.3.1, prepare your environment as follows:

[source,console,subs="verbatim,attributes"]
----
export GRAALVM_HOME=/Library/Java/JavaVirtualMac
