...
Both `create` and `drop` operations can be make idempotent by using `ifNotExists` and `ifExists` attributes.
...
WARNING: The `apply` operation loads all supported item types from the database, drops them and then creates all items of
         ...
         (Basically, none of the constraints defined for the lable `__Neo4jMigrationsLock`).
