...
Neo4j 4.4 introduces https://neo4j.com/docs/cypher-manual/current/indexes-for-search-performance/#indexes-future-indexes[future indexes], `RANGE` and `POINT` which replace the well known `BTREE` indexes of Neo4j 4.x. These "future" indexes are available from Neo4j 4.4 onwards but will not participate in any query planing in Neo4j 4.4. They exist merely for migration purposes in Neo4j 4.4: Neo4j 5.0 will not support `BTREE` indexes at all. This means a database that contains `BTREE` indexes _cannot_ be upgraded to Neo4j 5.0. Existing `BTREE` indexes need to be dropped prior to attempting the upgrade. This `ac.simons.neo4j.migrations.core.refactorings.MigrateBTreeIndexes` has been created for this purpose. It allows creation of matching "future" indexes and optionally dropping the indexes that are no longer supported in Neo4j 5.0 and higher prior to upgrading the store.

As with all the other refactorings, it can be used programmatically in your own application or through Neo4j-Migrations.

==== Preparing an upgrade to Neo4j 5 by creating future indexes in parallel
...
==== Preparing an upgrade to Neo4j 5 by replacing `BTREE` indexes with future indexes

The advantage to this approach is the fact that it won't need additional manual work before doing a store upgrade. However, the store upgrade should follow closely after dropping the old indexes and creating the replacement indexes as the latter won't participate in planing et. all prior to the actual upgrade to Neo4j 5.0 or higher.
...
[source,xml,subs="verbatim,attributes"]
.Annotation processor configured as processor fot the compiler plugin inside a Maven pom
----
<plugin>
    ...
    <configuration>
        ...
        <compilerArgs>
            <arg>-Aorg.neo4j.migrations.catalog_generator.default_catalog_name=V${next-migration-version}__Create_sdn_constraints.xml</arg>
...
`neo4j-migrations-{latest_version}.jar`:: Needed to generate the catalogs
`neo4j-migrations-annotation-processor-api-{latest_version}.jar`:: Contains the API and built-in annotations
`neo4j-migrations-annotation-processor-{latest_version}.jar` :: The processor itself
...
----
javac -proc:only \
-processorpath neo4j-migrations-{latest_version}.jar:neo4j-migrations-annotation-processor-api-{latest_version}.jar:neo4j-migrations-annotation-processor-{latest_version}.jar \
