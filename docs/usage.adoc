...
== Defining, asserting and applying catalogs
...
[source,xml]
----
include::catalog_example/V010__Assert_empty_schema.xml[]
----
...
----
include::catalog_example/V020__Create_person_name_unique.xml[]
----
...
[[V030__Fix_person_name_constraint_CE]]
.V030__Fix_person_name_constraint_CE.xml
----
include::catalog_example/V030__Fix_person_name_constraint_CE.xml[]
----

and for the enterprise edition, we can redefine the constraint like this:

[[V030__Fix_person_name_constraint_EE]]
.V030__Fix_person_name_constraint_EE.xml
----
include::catalog_example/V030__Fix_person_name_constraint_EE.xml[]
----

Note how we *can* `refer` to the constraint by ref in <<V030__Fix_person_name_constraint_EE>> and how we *must* use
`item` in <<V030__Fix_person_name_constraint_CE>>. The reason for that is that we refer to an older item only in the
