...
class DefaultMigrateBTreeIndexesIT {
	...
	Driver getDriver() {
		...
		return GraphDatabase.driver(this.neo4j.getBoltUrl(), AuthTokens.basic("neo4j", this.neo4j.getAdminPassword()));
	}
	...
	void shouldFindIndexes() {
		...
		try (Driver driver = getDriver(); Session session = driver.session()) {
			...
			assertThat(bTreeBasedIndexes.values()).hasSize(5)
				.extracting(Index::getName)
				.extracting(Name::getValue)
				.containsExactlyInAnyOrder("m_default_unique_constraint", "m_default_node_key_constraint",
						"m_default_index", "m_default_composite_index", "m_location_index");
		}
	}
	...
	void shouldFindConstraints() {
		...
		try (Driver driver = getDriver(); Session session = driver.session()) {
			...
			assertThat(constraints.values()).hasSize(2)
	...
	void shouldCreateParallelIndexes() {
		...
		try (Driver driver = getDriver(); Session session = driver.session()) {
			...
			assertThat(oldIndexes).containsExactlyInAnyOrder("m_default_index", "m_default_composite_index",
					"m_location_index");

			List<String> newIndexes = session.run(QUERY_GET_NEW_INDEXES).list(EXTRACT_NAME);
			assertThat(newIndexes).containsExactlyInAnyOrder("m_default_index_new", "m_default_composite_index_new",
					"m_location_index_new");
		}
	}
	...
	void shouldDropIndexes() {
		...
		try (Driver driver = getDriver(); Session session = driver.session()) {
			...
			List<String> newIndexes = session.run(QUERY_GET_NEW_INDEXES).list(EXTRACT_NAME);
			assertThat(newIndexes).containsExactlyInAnyOrder("m_default_index", "m_default_composite_index",
					"m_location_index");
		}
	}
	...
	void shouldUseTypeMapping() {
		...
		try (Driver driver = getDriver(); Session session = driver.session()) {
			DefaultMigrateBTreeIndexes refactoring = new DefaultMigrateBTreeIndexes(true, null, typeMapping,
					Collections.emptyList(), Collections.emptyList());
			Counters counters = refactoring.apply(new TestRefactoringContext(session));
			...
			Map<String, String> newIndexes = session.run(
					"SHOW INDEXES YIELD name, type, uniqueness WHERE name =~ 'm_.*' AND type <> 'BTREE' AND uniqueness <> 'UNIQUE'")
				.stream()
				.collect(Collectors.toMap(r -> r.get("name").asString(), r -> r.get("type").asString()));
			assertThat(newIndexes).hasSize(3)
