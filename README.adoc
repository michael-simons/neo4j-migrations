...
Migrates Neo4j databases.
  -a, --address=<address>   The address this migration should connect to. The
      ...
      --autocrlf            Automatically convert Windows line-endings (CRLF)
                              ...
                              checkin.
  -d, --database=<database> The database that should be migrated (Neo4j 4.0+).
...
| java.lang.String
| `null`
| An alternative user to impersonate during migration. Might have higher privileges than the user connected, which  will be dropped again after migration. Requires Neo4j 4.4+. Leave {@literal null} for using the connected user.
...
After installing at least GraalVM 21.3.0, prepare your environment as follows:
