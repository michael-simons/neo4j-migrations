...
class CatalogGeneratingProcessorTest {
	...
	void shouldInvokePrimaryKeyNameGeneratorProper() {
		...
		Compilation compilation = getCompiler()
			.withProcessors(new CatalogGeneratingProcessor(() -> "foo.txt", generator))
	...
	void shouldUseDefaultCatalogNameGeneratorOnBogusThings(String fqn) {
		...
		Compilation compilation = getCompiler()
			...
			.withOptions("-Aorg.neo4j.migrations.catalog_generator.catalog_name_supplier=" + fqn)
			.compile(getJavaResources("ac/simons/neo4j/migrations/annotations/proc/sdn6/ignored"));

		CompilationSubject
			.assertThat(compilation).succeeded();
		CompilationSubject
			.assertThat(compilation).hadWarningContaining(String.format("Could not load catalog name generate of type `%s`, using default.", fqn));
	}
