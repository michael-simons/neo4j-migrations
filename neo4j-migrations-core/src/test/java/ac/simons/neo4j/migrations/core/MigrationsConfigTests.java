...
class MigrationsConfigTests {
	...
	void shouldConfigureDefaultClasspathPackage() {
		...
		assertThat(MigrationsConfig.builder().build().getLocationsToScan())
			.containsExactlyElementsOf(Defaults.LOCATIONS_TO_SCAN);
	}
	...
	void shouldWarnLog() {
		...
		MigrationsConfig config = MigrationsConfig.builder().withLocationsToScan().build();
		Logger logger = Logger.getLogger(UUID.randomUUID().toString());
		LogCollector logCollector = new LogCollector();
		logger.addHandler(logCollector);
	...
	class SchemaDatabase {
		...
		void shoulldBeConfigurableIndependendFromDatabase() {
			...
			MigrationsConfig config = MigrationsConfig.builder().withSchemaDatabase("sd").withDatabase("d").build();
