...
class ConnectedCommandTest {
	...
	void noStackTraceNoCatalogException() {
		IllegalArgumentException foo = new IllegalArgumentException("foo");
	...
	void aCatalogExceptionIsACatalogException1() {
		...
		Constraint constraint = Constraint.forNode("F").named("f").key("f");
		RenderConfig renderConfig = RenderConfig.create().forVersionAndEdition(Neo4jVersion.LATEST, Neo4jEdition.COMMUNITY);
		try {
	...
	void aCatalogExceptionIsACatalogException2() {
		Renderer<Constraint> renderer = Renderer.get(Renderer.Format.CYPHER, Constraint.class);
		Constraint constraint = Constraint.forNode("F").named("f").unique("a", "b");
		RenderConfig renderConfig = RenderConfig.create().forVersionAndEdition(Neo4jVersion.V3_5, Neo4jEdition.COMMUNITY);
		try {
			renderer.render(constraint, renderConfig);
			Assertions.fail("Exception not thrown as expected.");
