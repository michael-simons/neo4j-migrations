...
satisfied (logically chained with `AND`).
...
The Neo4j edition can be required with `// assume that edition is enterprise` or `// assume that edition is community`.
...
NOTE: Why only preconditions for scripts? Since we offer <<concepts_migrations_java-based,full programmatic access>> to migrations
      ...
      it would be duplicate work if we take the decision from your hand.
      You are completely free inside a programmatic refactoring not to do anything in a given context. The migration will be
      dutifully recorded nevertheless.
      On the other hand, that would not be possible in a (simple) Cypher-script.
