...
A restricted set of options can be given after the description, prior to the suffix. Allowed options are: `runAlways` and `runOnChange`. These options are mutually exclusive and only applied to Cypher based migrations:

[source,console]
----
V1_2_3__Add_last_name_index(runAlways).cypher
----

The round brackets are required in this pattern.
...
[source,xml,indent=0,tabsize=2]
----
include::../neo4j-migrations-core/src/test/resources/catalogbased/manual/V42__Rename_labels.xml[]
----
...
This applies to both Cypher scripts outside an application (in the file system) and inside an application (as resources). A restricted set of options can be given after the description, prior to the suffix. Allowed options are: `runAlways` and `runOnChange`. These options are mutually exclusive.
...
* `runAlways`: Always run this migration, even if it has been applied before.
* `runOnChange`: Make this migration mutable and re-run it everytime it has been changed.

WARNING: Cypher-based migrations scripts are considered to be immutable once applied, except for those explicitly marked `runOnChange`.
