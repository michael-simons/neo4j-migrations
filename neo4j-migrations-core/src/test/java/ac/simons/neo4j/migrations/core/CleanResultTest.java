...
class CleanResultTest {
	...
	void shouldFormatDatabase() {
		...
		assertThat(result.prettyPrint()).isEqualTo(
			"Deleted chain a (0 nodes and 0 relationships in total) and 0 constraints from `x`.");
	}
	...
	void shouldUseCorrectNumbers() {
		...
		assertThat(result.prettyPrint()).isEqualTo(
			"Deleted chain a (1 nodes and 2 relationships in total) and 3 constraints from `x`.");
	}
}
