...
include::{manualIncludeDir}/README.adoc[tags=abstract]
...
Core:: The core module, providing an API to run both Cypher script and Java based migrations. The API includes builders
for configuration. The core is released as a Multi-Release-Jar, so that it behaves nicely on the module-path on
JDK 17. Be aware that we sealed all interfaces not intended to be implemented by you when running on JDK 17.
CLI:: A command line tool that supports every interaction that the core-module provides. Native binaries are available
for Linux, macOS and Windows. If you want to use Java based migrations in the CLI, you must use the JVM distribution.
This is an ideal tool to be put into CI/CD not based on Maven or Gradle.
Maven-Plugin:: A Maven-plugin that hooks clean, apply and verify operations into the appropriate Maven lifecycles. Use this
to apply migrations during your build.
...
into a fully configured Migrations instance that will be applied on application start. Scripts will be searched sane default
location.
...
The original idea of Neo4j-Migrations was conceived when working on integrating https://github.com/spring-projects/spring-data-neo4j[Spring Data Neo4j] (back then SDN/RX)
into https://www.jhipster.tech[JHipster]. We needed some Nodes, constraints and relationship to be present in the Neo4j database
...
Neo4j-Migrations filled that gab in early 2020 and has grown ever since.
