...
public interface AddSurrogateKey extends CustomizableRefactoring<AddSurrogateKey> {
	...
	static AddSurrogateKey toNodes(String primaryLabel, String... additionalLabels) {
		...
		if (labels.stream().anyMatch(Strings::isBlank)) {
	...
	static AddSurrogateKey toRelationships(String type) {
		...
		return new DefaultAddSurrogateKey(DefaultAddSurrogateKey.Target.RELATIONSHIP, Collections.singleton(type), DEFAULT_PROPERTY_NAME, DEFAULT_KEY_GENERATOR);
	}
	...
	static AddSurrogateKey toNodesMatching(String customQuery) {
		...
		if (Strings.isBlank(customQuery)) {
