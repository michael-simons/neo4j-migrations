...
class MigrationsProcessorTest {
	...
	class ClassDiscovering {
		...
		void shouldFindClasses() throws IOException {
			...
			indexer.indexClass(MigrationsProcessorTest.SomeMigration.class);

			var classes = MigrationsProcessor.findClassBasedMigrations(
				List.of(MigrationsProcessorTest.class.getPackageName()), indexer.complete());
